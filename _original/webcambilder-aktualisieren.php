...
// Da zuerst die leere Datei mit Dateiname webcam.jpg mit 0 kb per FTP übertragen wird, dann 2-4 s Upload-/Verarbeitungszeit der ca. 120 kb, dann der Kopier-/Nachbearbeitungsvorgang (ca. 80 kb), ergibt es zu oft Bildanzeige-/Webseitenfehler, wenn auf filesize nicht abgefragt werden würde. Falls keine neue Aufnahme (= Datei: webcam.jpg) vorhanden ist, muss nichts gemacht (aktualisieren/austauschen) werden. Die webcam.jpg wird nach Verarbeitung gelöscht und die Webcam erstellt diese Datei bei neuer Aufnahme erneut.

// Gibt es eine Aufnahme, die die Webcam übertragen hat? Ist die Datei lesbar und größer als 80 kb?
if( file_exists($tenniswebcam_letztesBild) and (is_readable($tenniswebcam_letztesBild)) and (filesize($tenniswebcam_letztesBild) > 80000) ){
...
// BUGFIX: !empty() auf einen String ist immer true und damit sinnlos. Stattdessen is_dir() und scandir() prüfen.
...
if( is_dir($padelordner.'/'.$jahr.'/'.$monat.'/'.$tag) ){
	...
	foreach ($bilder as $bild){
		...
		// Ist die Datei größer als 20 kb? Weil zuerst der Dateiname mit 0 kb übertragen wird, dann 2-4 s Verarbeitungszeit, dann der Kopiervorgang die restlichen kb schnell überträgt.
		if( is_file($bildpfad) and (filesize($bildpfad) > 20000) ){
	...
	if( !empty($bilder_Padel_00) and !empty($bilder_Padel_01) ){
		...
		// Aufgrund absteigender Sortierung bei scandir (s.o.) ist das aktuellste Bild der erste Index, also [0].
